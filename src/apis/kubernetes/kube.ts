...
import { Configuration, CoreV1Api, createConfiguration, CustomObjectsApi, dumpYaml, KubeConfig, loadYaml, RequestContext, ResponseContext, ServerConfiguration, V1ObjectMeta } from '@kubernetes/client-node';
...
export class Kubernetes extends Utils {
    ...
    public async namespaceExists(name: string): Promise<boolean> {
        ...
        try {
            ...
            if (response && response.metadata && response.metadata.name === name) {
                return true;
            }
    ...
    public async getOpenshiftRoute(name: string, namespace: string): Promise<string> {
        const customObjectsApi = this.kubeConfig.makeApiClient(CustomObjectsApi);
        try {
            const { body: openshiftRoute } = await customObjectsApi.getNamespacedCustomObject({group: 'route.openshift.io', version: 'v1', plural: 'routes', name: name, namespace: namespace});
    ...
    public async getDeveloperHubSecret(namespace: string, secretName: string, keyName: string): Promise<string> {
        ...
        try {
            ...
            if (secret.data && secret.data[keyName]) {
                ...
                const secretValue = Buffer.from(secret.data[keyName], 'base64').toString('utf-8');
                return secretValue;
            } else {
